<template>
  <div class="coupon-item">
    ...
    >
      <div
        class="coupon-type"
        :class="{ 'type-discount': item.couponType == '2' }">
    ...
    <div class="coupon-state">
      ...
      <div>
        <span class="coupon-value" :class="{fc666: item.useStatus == -1, 'small': item.amount && item.amount.toString().length>6}" v-if="item.amount"
          >{{ unitConvert(item.amount).num }}<span class="coupon-unit">{{
...
.coupon-item {
  ...
  .coupon-body {
    ...
    .coupon-type {
      ...
      background: linear-gradient(180deg, #FADBAD  0%, #FADBAD  100%);
