  ...
  <div class="message">
    <div class="message-content">
      ...
      <div
              ...
              :key="index"
              @click="viewUrl(item.externalUrl)"
      >
        <div class="eventoffer-content">
          ...
          <div class="eventoffer-litle">
            ...
            <span class="date">{{item._date}}</span>
          </div>
    ...
    <div class="no-record" v-if="!enlistpromotionList.length && !enlistpromotionList.length"></div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      viewUrl(url){
        ...
        // this.$client.pushWindow("/pages/webView/webView?url=" + encodeURIComponent(url),{reLaunch:true});
        //  }else{
      ...
      async getPromotionlist() {
        ...
        this.isGet = true;
        let { data } = await this.$client.rpc(apis.APLUS_LIST, {
          page,
          pageSize: 10,
          state: 1,
          deleted: 0,
        ...
        if (data.success) {
          ...
          this.isUpNoMore = data.total <= (page * 10);
          if (page == 1) {
